...
create-test:
    clickhouse-client --query "`cat test/test-beryl.sql`"
    clickhouse-client --query "`cat test/test-beryl-insert.sql`"

create-test-macos:
    cat test/test-beryl.sql | docker run -i --rm --link tesseract-clickhouse-server:clickhouse-server yandex/clickhouse-client --host clickhouse-server
    cat test/test-beryl-insert.sql | docker run -i --rm --link tesseract-clickhouse-server:clickhouse-server yandex/clickhouse-client --host clickhouse-server
